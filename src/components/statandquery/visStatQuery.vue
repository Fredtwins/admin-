    ...
    <row>
      <i-col span="21">
        <Breadcrumb class="ml32">
          ...
          <Breadcrumb-item class="blue">能见度查询</Breadcrumb-item>
        </Breadcrumb>
        <Collapse class="aa">
          <Panel name="1">
            条件筛选
              ...
              <Form ref="formInline" :model="formInline" inline :label-width="80">
                <Form-item label="站点名称" prop="station_name">
                  <Select v-model="formInline.station_name" clearable style="width:200px">
                    <Option v-for="item in stationList" :value="item.name"/>
                  </Select>
                </Form-item>
                <Form-item label="时间段">
...
<script>
  export default {
    data () {
      return {
        collectionLogListUrl: 'gmmeteo/datacenter/vis/list',
        stationListUrl:'gmmeteo/datacenter/station/vis',
        timeAgo:'',
        timeNew:'',
        formInline: {
          station_name : "",
    ...
    methods: {
      ...
      getVisStationInfo:function () {
        var that = this;
        var params = {};
        params.data = {
          system_id : parameters.system_id,
          searchValue : {territory_name : "高明区"}
        };
        params.token = this.getKey('token');
      ...
      exportExcel: function() {
        if (this.formInline.datetime1 != '' && this.formInline.datetime2 != '') {
          window.open(parameters.domain + 'gmmeteo/datacenter/export/visStatQuery?station_name=' + this.formInline.station_name +
            '&datetime=' + this.formInline.datetime1 + '|%23|' + this.formInline.datetime2);
        } else {
          this.$Notice.warning({
            title: '请选择时间'
          });
