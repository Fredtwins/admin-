    ...
    <row>
      <i-col span="21">
        <Breadcrumb class="ml32">
          ...
          <Breadcrumb-item class="blue">大气电场统计查询</Breadcrumb-item>
        </Breadcrumb>
        <Collapse class="aa">
          <Panel name="1">
            条件筛选
...
<script>
  export default {
    data () {
      return {
        collectionLogListUrl: 'gmmeteo/datacenter/aef/list',
        timeAgo:'',
        timeNew:'',
        formInline: {
          datetime1 : "",
    ...
    methods: {
      ...
      exportExcel: function() {
        if (this.formInline.datetime1 != '' && this.formInline.datetime2 != '') {
          window.open(parameters.domain + 'gmmeteo/datacenter/export/aefStatQuery?datetime=' + this.formInline.datetime1 + '|%23|' + this.formInline.datetime2);
        } else {
          this.$Notice.warning({
            title: '请选择时间'
          });
